...
parser.add_argument(
    "--save",
    type=str,
    default="./save_model/" + str(time.strftime("%Y-%m-%d-%H:%M:%S")) + "-",
    help="save path",
)
parser.add_argument(
    "--es_patience",
...
parser.add_argument("--head", type=int, default=1, help="head")
...
log_filename = f"{log_dir}/{dataset_name}_{now}.log"
# 配置 logging
logging.basicConfig(
...
class trainer:
    def __init__(
        ...
        self.model = DMT(
            device, input_dim, channels, num_nodes, input_len, output_len, dropout,lamda,layers,head
        )
        ...
        self.scaler = scaler
        self.clip = 5
        logging.info("The number of parameters: {}".format(self.model.param_num()))
...
def main():
    ...
    for i in range(1, args.epochs + 1):
        ...
        logging.info(log.format(i, (t2 - t1)))
        ...
        if mvalid_loss < loss:
            ...
            logging.info("###Update tasks appear###")
            if i < 100:
                ...
                bestid = i
                epochs_since_best_mae = 0
                logging.info(f"Updating! Valid Loss:{ mvalid_loss}, epoch:{i}" )
                ...
                if np.mean(amae) < test_log:
                    ...
                    torch.save(engine.model.state_dict(), path + "best_model.pth")
                    epochs_since_best_mae = 0
                    logging.info(f"Test low! Updating! Test Loss: {np.mean(amae)}")
                    logging.info(f"Test low! Updating! Valid Loss: {mvalid_loss}")
                    bestid = i
    ...
    logging.info("The epoch of the best result：%d" % bestid)
    logging.info("The valid loss of the best model: %s" % str(round(his_loss[bestid - 1], 4)))
    ...
    for i in range(args.output_len):
        ...
        log = "Evaluate best model on test data for horizon {:d}, Test MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}, Test WMAPE: {:.4f}"
        logging.info(
            log.format(i + 1, metrics[0], metrics[2], metrics[1], metrics[3])
        )
